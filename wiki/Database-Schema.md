...
Full entity reference. All entities across all build phases. The executable Prisma schema is in `scaffold/schema.prisma`.
...
### `subscriptions`
One subscription per tenant. Created automatically on teacher registration.
...
| feature_flags | jsonb | `{ai, analytics, team, branding}` |
| renews_at | date? | |
| grace_until | date? | Phase 2 — set after cancellation |
...
| disengagement_flag | bool | Phase 4 |
...
| tenant_id | uuid FK | |
| teacher_id | uuid FK | |
| student_id | uuid FK? | 1-on-1 only |
| classroom_id | uuid FK? | Group only |
...
| annotation_stroke_key | string? | Phase 3 |
...
### `board_strokes`
One record per board page — points to the binary stroke blob.

| Field | Type | Notes |
|---|---|---|
| id | uuid PK | |
| page_id | uuid FK unique | |
| strokes_url | string? | R2 binary blob URL |
...
## RT Collaboration layer (Phase 3)

### `board_commands`
Command log for shared undo/redo.
...
### `badges`
Seeded catalogue — not mutated by tenants.
...
### `template_clones`
Audit trail for template cloning.
...
The following entities must **never** be hard-deleted. Always call `softDelete(model, id)`:
...
Prisma middleware automatically filters out `deleted_at IS NOT NULL` rows from all `findMany` queries on these models. See [[Prisma-Middleware]].
